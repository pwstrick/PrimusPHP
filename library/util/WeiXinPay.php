...
//    private $KEY = WEIXIN_KEY;
    ...
    public function __construct() {
        ...
        $this->param['nonce_str'] = md5(uniqid('chelun'));
     ...
     * jsapi_ticket 参数设置
     */
    public function getJsapiParam($ticket, $url) {
        $param = [
            'jsapi_ticket' => $ticket,
            'timestamp' => time(),
        ...
        \InitPrimus::writeLog(json_encode($param));
        $str = [];
        ...
        $unsignKey = join('&', $str);
        $param['paySign'] = strtolower(sha1($unsignKey));
        return $param;
    }
    /**
     * 生成jsapi需要调用的参数
    ...
    public function getPayParam(){
        ...
        if(isset($this->param['prepay_id'])) {
            $param['package'] = 'prepay_id='.$this->param['prepay_id'];
        }
