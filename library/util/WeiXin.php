...
//        curl_setopt($resource, CURLOPT_SSL_VERIFYHOST, 0);
        ...
        if (curl_errno($resource)) {
            \InitPrimus::writeLog('Curl error: ' . curl_error($resource));
        }
        curl_close($resource);
        \InitPrimus::writeLog($data);//返回值打log
        return $data;
    }
     ...
     *          Url => 点击图文消息跳转链接
     *      )
     * @param bool $isAuth 链接地址是否需要验证
     * @param string snsapi_userinfo | snsapi_base
     *      snsapi_userinfo 可以用来获取用户信息
     *      snsapi_base 可以用来获取openid
     * @return string 返回消息体
     */
    public function createTuWenMsg($fromUserName, $toUserName, $items = array(), $isAuth=true, $scope = 'snsapi_base', $state = 'from_weixin')
    {
        if (!is_array($items)) {
            return '';
        }
...
MSG;
     ...
     * @return string 返回消息体
     */
    public function createWordMsg($fromUserName, $toUserName, $msg = '')
    {
        $msg = strval($msg);
        if (!$msg) {
            $msg = '感谢您关注车轮查违章，请点击菜单了解更多';
        }
        $createTime = time();
        $txtMsg = <<<TXTMSG
<xml>
...
TXTMSG;
     ...
     * @return array|bool 如果成功信息中也会包含openid
     */
     ...
     * @return bool|array
     */
    public function getUserByWebAccessToken($web_access_token, $openid)
    {
     ...
     * @return array | null
     */
    public function getUserByAccessToken($access_token, $openid)
    {
     ...
     * @param $sceneId 场景值id
     * @param $is_limit 是否是永久
     * @return bool|mixed
     */
    public function getQrcodeTicket($access_token, $sceneId, $is_limit=true, $expire=null)
    {
        ...
        $action_name = $is_limit ? 'QR_LIMIT_SCENE' : 'QR_SCENE';
        $raw = array(
            'action_name' => $action_name,
            'action_info' => array(
                'scene' => array(
                    'scene_id' => $sceneId,
                    //'scene_str' => '15821253928'
                )
            )
        );
        if(!empty($expire)) {
            //有效时间
            $raw['expire_seconds'] = $expire;
        }
